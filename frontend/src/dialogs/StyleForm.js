...
import {Dialog, DialogTitle, DialogContent, DialogActions, Button, TextField, FormControl, InputLabel, Select, MenuItem,
  List, ListItem, ListItemText, IconButton, Box, Typography, Chip} from '@mui/material';
...
class StyleForm extends Component {
    ...
    componentDidUpdate(prevProps) {
        if (prevProps.show !== this.props.show || prevProps.style !== this.props.style) {
            this.initializeFormData();
        }
    }
    ...
    loadKleidungstypen = async () => {
        try {
            ...
            this.setState({kleidungstypen: allTypes});
        } catch (error) {
     ...
     */
    handleConstraintSave = (constraintType, constraintData) => {
     ...
     */
    handleConstraintDelete = (type, index) => {
        setFormData(prev => {
    ...
    render() {
        ...
        const {formData, activeDialog, selectedConstraint} = this.state;
